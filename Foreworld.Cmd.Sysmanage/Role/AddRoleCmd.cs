...
{
    [Implementation("addRole", Description = "添加角色", Version = "1.0.0.0")]
    class AddRoleCmd : AbstractBaseCmd
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(AddRoleCmd));

        [Parameter("rolegroupid", "角色组名称", Required = true, Regexp = "^[A-Z_]{2,20}$", RegexpInfo = "角色组名称为必填项")]
        ...
        public override object Execute(Parameter @parameter)
        {
            ...
            if (__startime >= __endtime) return Util.ExceptionLog(Resource.AddRoleCmd_Err);

            List<SqlParameter> __sps = new List<SqlParameter>();

            SqlParameter __sp = null;

            __sp = new SqlParameter("@tab_s_rolegroup_id", SqlDbType.VarChar, 20);
            __sp.Value = __parameters["rolegroupid"];
            __sps.Add(__sp);
            ...
            __logInfo.Msg = Resource.AddRoleCmd_Sql;
            _log.Debug(__logInfo);
#endif
            #endregion
            ...
            try
            {
                __ds = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.Text, Resource.AddRoleCmd_Sql, __sps.ToArray());
            }
            catch (Exception @ex)
            {
                __logInfo.Code = Resource.AddRoleCmd_Err_Code;
                __logInfo.Msg = Resource.AddRoleCmd_Err;
                _log.Error(__logInfo, @ex);
            }

            //找不到新记录
            if (__ds == null) return Util.ExceptionLog(Resource.AddRoleCmd_Err);
            ...
            if (__rows.Count == 0)
            {
                __ds.Clear();
                __ds.Dispose();
                return Util.ExceptionLog(Resource.AddRoleCmd_Err);
            }
            #endregion
            ...
            _log.Debug(__logInfo);
#endif
            __logInfo.Msg = string.Format(Resource.AddRoleCmd, __parameters["rolename"]);
            _log.Info(__logInfo);
