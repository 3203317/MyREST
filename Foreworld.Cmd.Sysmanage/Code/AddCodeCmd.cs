...
{
    [Implementation("addCode", Description = "添加代码", Version = "1.0.0.0")]
    class AddCodeCmd : AbstractBaseCmd
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(AddCodeCmd));
        ...
        [Parameter("p_code", "父编码", Required = true, Regexp = "^[A-Z0-9_]{1,20}$", RegexpInfo = "父编码为必填项")]
        ...
        public override object Execute(Parameter @parameter)
        {
            ...
            __sp.Value = __parameters["p_code"];
            __sps.Add(__sp);
            ...
            __logInfo.Msg = Resource.AddCodeCmd_Sql;
            _log.Debug(__logInfo);
#endif
            #endregion
            ...
            try
            {
                __ds = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.Text, Resource.AddCodeCmd_Sql, __sps.ToArray());
            }
            catch (Exception @ex)
            {
                __logInfo.Msg = Resource.AddCodeCmd_Err;
                __logInfo.Code = Resource.AddCodeCmd_Err_Code;
                _log.Error(__logInfo, @ex);
            }

            //找不到新记录
            if (__ds == null) return Util.ExceptionLog(Resource.AddCodeCmd_Err);
            ...
            if (__rows.Count == 0)
            {
                __ds.Clear();
                __ds.Dispose();
                return Util.ExceptionLog(Resource.AddCodeCmd_Err);
            }
            #endregion
            ...
            __logInfo.Msg = string.Format(Resource.AddCodeCmd, __sps[0].SqlValue);
            _log.Info(__logInfo);
