...
{
    class DownloadInterceptorManager
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(DownloadInterceptorManager));
        private static LogInfo _logInfo = new LogInfo();

        private DownloadInterceptorManager()
        {
            _logInfo.Msg = "DownloadInterceptorManager初始化...";
            _log.Info(_logInfo);
            Init();
        }

        private static volatile DownloadInterceptorManager _instance;
        private static object _syncRoot = new Object();

        public static DownloadInterceptorManager INSTANCE
        {
            get
            {
                if (_instance == null)
                {
                    lock (_syncRoot)
                    {
                        if (_instance == null) _instance = new DownloadInterceptorManager();
                    }
                }
                return _instance;
            }
