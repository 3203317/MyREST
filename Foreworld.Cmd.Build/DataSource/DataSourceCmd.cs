...
{
    [Implementation("dataSource", Description = "数据源服务", Version = "1.0.0.0")]
    class DataSourceCmd : AbstractBaseCmd
    {
        private LogInfo _logInfo;

        private static readonly ILog _log = LogManager.GetLogger(typeof(DataSourceCmd));
        ...
        public override object Execute(Parameter @parameter)
        {
            ...
            {
                _logInfo.Code = Resource.DataSourceCmd_Err_1;
                _log.Error(_logInfo, @ex);
            }
            #endregion

            #region 判断ds是否为空
            if (__ds == null) return Util.ExceptionLog(Resource.DataSourceCmd_Err_1);
            #endregion
        ...
        private string GetDSQLFromDB(string @id)
        {
            ...
            try
            {
                __ds = SqlHelper.ExecuteDataSet(ConnectionString, CommandType.Text, Resource.DataSourceCmd_Sql, __sps.ToArray());
            }
            catch (Exception @ex)
            {
#if DEBUG
                _logInfo.Msg = Resource.DataSourceCmd_Err_2;
                _log.Error(_logInfo, @ex);
#endif
            }
            #endregion
            ...
            if (__dstype == "1")
                __sql = Resource.DataSourceCmd_Sql_1 + __sql + Resource.DataSourceCmd_Sql_2;
            else
                __sql += Resource.DataSourceCmd_Sql_3;
