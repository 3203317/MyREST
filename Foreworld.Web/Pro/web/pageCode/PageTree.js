...
define(["jquery",
	...
	var _tree = function($params){
		...
		__tree.setOnSelectStateChange(function($id){
			var __params = {
				id: 2 == this.getUserData($id, "nodetype") ? $id.substring(1) : 0,
				v1: this.getUserData($id, "v0"),
				v2: this.getUserData($id, "v1")
			};
		...
		var __loadData = function($data){	
			/* 树的根对象 */
			var __treeObj = { id: "0", item: [], text: "根", select: 1 };	
			_data2TreeObject($data,0,__treeObj);
			__tree.loadJSONObject(__treeObj, function(){
		...
		$.ajax({
			type: "get",
			url: "../../Api.ashx?command=listPageTree",
			dataType: "json",
			async: true,
